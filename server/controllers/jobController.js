...
export const applyJob = async (req, res, next) => {
    try {
        const { jobId, name, email, contact, about } = req.body;

        if (!jobId || !name || !email) {
            return next("Please provide jobId, name, and email");
        }

        const jobApplication = {
            jobId,
            applicant: req.body.user.userId,
            name,
            email,
            contact,
            about,
...
export const getUserJobApplications = async (req, res, next) => {
    try {
        ...
        const applications = await JobApplications.find({ applicant: userId }).populate({
            path: 'jobId',
